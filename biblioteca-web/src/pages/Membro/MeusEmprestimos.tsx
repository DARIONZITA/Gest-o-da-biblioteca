...
import { BookMarked, Search, RefreshCw, AlertTriangle, CheckCircle } from 'lucide-react';
import { Emprestimo, StatusEmprestimo } from '@/types/emprestimo';
import { emprestimosService } from '@/services/emprestimosService';
import { toast } from '@/utils/toast';
import { MAX_RENEWALS } from '@/constants';
...
const MeusEmprestimos: React.FC = () => {
  ...
  const fetchEmprestimos = async () => {
    try {
      setLoading(true);
      const data = await emprestimosService.getMeusEmprestimos();
      setEmprestimos(data);
    } catch (error: any) {
  ...
  const filtered = emprestimos.filter(e => {
    const matchesSearch = e.tituloLivro.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <MemberLayout>
      <div className="space-y-6">
        <div className="animate-slide-up">
          <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Meus Empréstimos</h1>
          <p className="text-sm text-gray-400 mt-0.5 font-medium">Acompanhe os seus empréstimos de livros</p>
        </div>
          ...
          {loading ? (
            ...
            filtered.map((emp) => (
              <Card key={emp.id} padding="lg" hover className="group">
                <div className="flex items-start justify-between gap-4">
                  ...
                  {emp.status === 'ATIVO' && emp.qtdRenovacoes < MAX_RENEWALS && (
            ...
            <Card padding="lg">
              <EmptyState
                ...
                message={searchTerm ? 'Tente buscar com outros termos' : 'Você ainda não tem empréstimos'}
              />
            </Card>
          )}
        </div>
